function Run_For_Get_BrBt(Input, k)
...
fid = fopen('Run_For_Get_BrBt.vbs','w');
...
fprintf(fid, ['\nCall study.GetModel(0).GetStudy(',num2str(k-1),').GetCondition("' ,motion, '").SetValue("InitialRotationAngle", ',ini_angle_str,')']);   %Initial angle
...
fprintf(fid, ['\nCall study.GetDataManager().GetGraphModel("',num2str(T(k)),'Nm@',num2str(rpm(k)),'rpm_br").WriteTable("',current_path 'Br/' ,num2str(T(k)),'Nm@',num2str(rpm(k)),'rpm_br','.csv")']);    
...
fprintf(fid, ['\nCall study.GetDataManager().GetGraphModel("',num2str(T(k)),'Nm@',num2str(rpm(k)),'rpm_bt").WriteTable("',current_path 'Bt/' ,num2str(T(k)),'Nm@',num2str(rpm(k)),'rpm_bt','.csv")']); 

fprintf(fid, ['\nSet tabledata = study.GetModel(0).GetStudy(',num2str(k-1),').GetResultTable().GetData("Torque")']);
fprintf(fid, ['\nCall tabledata.WriteTable("',current_path,'Torque/',num2str(T(k)),'Nm@',num2str(rpm(k)),'RPM_Torque','.csv", "Time")']);    
